...
use spectre_protocol::w3gs::{ActionBlock, ids};
use spectre_spectator::body::ReplayBody;
...
fn parity_wire_p1_2_replay_contains_real_statstring() {
    ...
    let decoded = spectre_protocol::decode_statstring(stat_bytes);
    assert!(
        decoded.len() >= 14,
...
fn parity_wire_p1_8_relay_receives_in_game_chat_and_game_over() {
    ...
    match cmd {
        spectre_spectator::RelayCmd::ViewerChat { sender, text } => {
            assert_eq!(sender, "P1");
    ...
    match cmd {
        spectre_spectator::RelayCmd::ViewerChat { text, .. } => {
