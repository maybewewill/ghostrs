...
impl GameState {
    pub fn handle_chat_to_host(&mut self, conn_id: u64, payload: &Bytes) {
        ...
        match parse_command(trigger, &chat.message) {
            ...
            None => {
                ...
                if let Ok(b) = spectre_protocol::w3gs::outgoing::chat_from_host(
                    pid,
                    &to_pids,
                    chat.flag,
                    &chat.extra,
    ...
    pub fn run_command(&mut self, pid: u8, caller_name: &str, cmd: ChatCommand) {
        match cmd {
            ...
            ChatCommand::Version => {
                self.send_chat_to(
                    pid,
                    "Spectre v0.2.0 (High-Performance Async Warcraft III Hostbot)",
                );
            }
            ChatCommand::Ping => {
            ...
            ChatCommand::Download(name) => {
                ...
                if let Some(tpid) = target_pid {
                    ...
                    self.send_to(
                        tpid,
                        spectre_protocol::w3gs::outgoing::start_download(self.host_pid()),
                    );
                } else {
                    self.send_chat_to(pid, &format!("No player matching [{name}]."));
                }
            ...
            ChatCommand::FpPause => {
                if let (Some(fake_pid), GamePhase::Playing) = (self.fake_player_pid, self.phase) {
                    let act = spectre_protocol::w3gs::ActionBlock {
                        pid: fake_pid,
    ...
    pub fn send_chat_to(&mut self, pid: u8, message: &str) {
        ...
        if matches!(self.phase, GamePhase::Lobby | GamePhase::Countdown { .. }) {
            ...
            if let Ok(b) =
                spectre_protocol::w3gs::outgoing::chat_from_host(from, &[pid], 0x10, &[], msg)
            {
                self.send_to(pid, b);
            }
        } else {
            ...
            if let Ok(b) =
                spectre_protocol::w3gs::outgoing::chat_from_host(from, &[pid], 0x20, &extra, msg)
            {
                self.send_to(pid, b);
            }
        }
...
mod tests {
    ...
    async fn trigger_question_mark_gets_the_command_trigger_reply() {
        ...
        assert!(
            sent.contains(&spectre_protocol::w3gs::ids::CHAT_FROM_HOST),
