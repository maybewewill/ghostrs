...
impl GameState {
    ...
    pub fn handle_keepalive(&mut self, conn_id: u64, payload: &Bytes) {
        ...
        let checksum = match spectre_protocol::w3gs::incoming::decode_keepalive(payload) {
            Ok(cs) => cs,
            Err(_) => return,
        };
    ...
    pub fn begin_playing(&mut self) {
        ...
        if let Some(r) = &self.relay {
            ...
            let stat_string = spectre_protocol::encode_statstring(&raw);

            let snap = spectre_protocol::dotatv::GameStartSnapshot {
    ...
    pub fn save_game_data(&mut self) {
        ...
        if let Some(dota) = &self.dota {
            ...
            let records: Vec<spectre_store::DotAPlayerRecord> = dota
                .players
                .values()
                .map(|p| spectre_store::DotAPlayerRecord {
                    colour: p.colour,
                    name: p.name.clone(),
                    hero: p.hero.clone(),
                    kills: p.kills,
...
mod tests {
    ...
    async fn oversized_action_batches_are_relayed_and_recorded_in_order() {
        ...
        st.relay = Some(spectre_spectator::RelayHandle::new(relay_tx));
        ...
        while let Ok(cmd) = relay_rx.try_recv() {
            match cmd {
                spectre_spectator::RelayCmd::GameStart(_) => saw_game_start = true,
                spectre_spectator::RelayCmd::PlayerInfo { .. } => saw_player_info = true,
                spectre_spectator::RelayCmd::GameBlock(b) => relay_packets.push(b),
    ...
    async fn game_actions_chat_and_leavers_are_recorded_in_replay_body() {
        ...
        let mut rep = spectre_spectator::ReplayBody::new(1, "iCCupHost");
    ...
    fn actions_sent_before_the_game_starts_kick_the_sender() {
        ...
        assert_eq!(p.left_code, spectre_protocol::w3gs::ids::PLAYERLEAVE_LOST);
        assert!(
            st.actions.is_empty(),
