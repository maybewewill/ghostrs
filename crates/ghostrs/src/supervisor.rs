...
use ghost_engine::{GameCmd, GameConfig, GameEvent, GameHandle, MapInfo, ParsedMap, spawn_game};
use ghost_net::{ConnEvent, UdpBroadcaster, spawn_conn, spawn_listener, spawn_listener_tagged};
...
impl Supervisor {
    ...
    fn handle_reconnect_connection(&self, conn_id: u64, stream: TcpStream, peer: SocketAddr) {
        ...
        tokio::spawn(async move {
            ...
            for i in 0..=buf.len().saturating_sub(13) {
                if buf[i] == ghost_protocol::gps::GPS_HEADER
                    && buf[i + 1] == ghost_protocol::gps::ids::RECONNECT
                {
                    pos = Some(i);
                    break;
                }
            ...
            let Ok(req) = ghost_protocol::gps::decode_reconnect(&payload) else {
                return;
            };
            ...
            let _ = link.try_send(ghost_protocol::gps::reject(
                ghost_protocol::gps::reject_reason::NOT_FOUND,
            ));
        });
    }
    ...
    fn handle_chat_command(&mut self, user: &str, text: &str) {
        ...
        match verb.to_lowercase().as_str() {
            ...
            "version" => {
                self.bnet.send(BnetCmd::SendChat(format!(
                    "/w {user} Ghost-RS v0.2.0 (High-Performance Async Warcraft III Hostbot)"
                )));
            }
            "dbstatus" => {
                self.bnet.send(BnetCmd::SendChat(format!(
...
mod tests {
    ...
    fn test_p1_6_lobby_status_event_updates_advert_and_sends_refresh() {
        ...
        let ev = ghost_engine::GameEvent::LobbyStatus {
            host_counter: 1234,
            slots_open: 7,
            slots_total: 10,
            human_players: 3,
    ...
    async fn test_p2_8_bnet_commands_handling() {
        ...
        assert!(matches!(cmd, ghost_bnet::BnetCmd::ClanInvitation(name) if name == "player1"));
        ...
        let cmd = bnet_cmd_rx.try_recv().unwrap();
        assert!(matches!(cmd, ghost_bnet::BnetCmd::GetClanList));
        let _ = bnet_cmd_rx.try_recv();

        // 3. !getfriends
        sup.handle_chat_command("admin", "!getfriends");
        let cmd = bnet_cmd_rx.try_recv().unwrap();
        assert!(matches!(cmd, ghost_bnet::BnetCmd::GetFriendsList));
        let _ = bnet_cmd_rx.try_recv();
        ...
        let cmd = bnet_cmd_rx.try_recv().unwrap();
        assert!(matches!(cmd, ghost_bnet::BnetCmd::ClanSetMotd(m) if m == "Welcome to the clan!"));
        let _ = bnet_cmd_rx.try_recv();

        // 6. !accept
        sup.handle_chat_command("admin", "!accept");
        let cmd = bnet_cmd_rx.try_recv().unwrap();
        assert!(matches!(cmd, ghost_bnet::BnetCmd::ClanAcceptInvite(true)));
        let _ = bnet_cmd_rx.try_recv();
    }
    ...
    async fn test_multi_lobby_port_pool_allocation_and_commands() {
        ...
        // Host first game
        sup.create_game("Game 1", "admin", ghost_protocol::GameVisibility::Public);
        assert_eq!(sup.games.len(), 1);
        assert_eq!(sup.games[0].port, 6113);

        let cmd1 = bnet_cmd_rx.try_recv().unwrap();
        if let ghost_bnet::BnetCmd::CreateGame { port, name, .. } = cmd1 {
        ...
        sup.create_game("Game 2", "admin", ghost_protocol::GameVisibility::Public);
        assert_eq!(sup.games.len(), 2);
        assert_eq!(sup.games[1].port, 6114);

        let cmd2 = bnet_cmd_rx.try_recv().unwrap();
        if let ghost_bnet::BnetCmd::CreateGame { port, name, .. } = cmd2 {
        ...
        if let ghost_bnet::BnetCmd::SendChat(msg) = chat_cmd {
        ...
        sup.create_game("Game 3", "admin", ghost_protocol::GameVisibility::Public);
        assert_eq!(sup.games.len(), 2);
