...
fn test_r3_language_string_helpers() {
    ...
    assert_eq!(
        lang::autostart_enabled(10),
        "Autostart enabled for 10 players."
    );
    assert_eq!(lang::autostart_disabled(), "Autostart disabled.");
    assert_eq!(
        lang::votekick_started("BadPlayer", 6),
        "Votekick started against [BadPlayer]. 6 votes needed."
    );
    assert_eq!(
        lang::votekick_passed("BadPlayer"),
        "Votekick against [BadPlayer] passed!"
    );
    assert_eq!(
        lang::votekick_cancelled("BadPlayer"),
        "Votekick against [BadPlayer] cancelled."
    );
