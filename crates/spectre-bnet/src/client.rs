...
use spectre_protocol::bncs::{BncsCodec, ids, incoming, outgoing};
...
pub enum BnetEvent {
    ...
    FriendsList(Vec<spectre_protocol::bncs::incoming::FriendListEntry>),
    ClanList(Vec<spectre_protocol::bncs::incoming::ClanMemberEntry>),
    ClanInviteReceived {
        clan_name: String,
        inviter: String,
        creation: bool,
...
fn advert_game_type(map: &MapAdvert, visibility: spectre_protocol::GameVisibility) -> [u8; 4] {
    ...
    if visibility == spectre_protocol::GameVisibility::Private {
